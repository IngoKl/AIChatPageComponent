...
class AIChatPageComponentOpenAI extends AIChatPageComponentLLM
{
    ...
    public static function createFromAIChatConfig(): self
    {
        try {
            ...
            $model = AIChatPageComponentConfig::get('openai_model') ?: 'gpt-3.5-turbo';
            
            $openai = new self($model);
            
            // Get API key from AIChat config
            $apiKey = AIChatPageComponentConfig::get('openai_api_key') ?: '';
            $openai->setApiKey($apiKey);
            
            // Get streaming setting
            $streaming = AIChatPageComponentConfig::get('openai_streaming') == "1";
